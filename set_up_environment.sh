...
echo "Choose the right container for the visualtransformer example"
export SIF=/appl/local/containers/sif-images/lumi-pytorch-rocm-6.2.1-python-3.12-pytorch-20240918-vllm-4075b35.sif

echo "Extend the container with a virtual environment and install h5py."
singularity exec $SIF bash -c '$WITH_CONDA && python -m venv visualtransformer-env --system-site-packages && source visualtransformer-env/bin/activate && python -m pip install h5py'
