...
def train_model(model, criterion, optimizer, train_loader, val_loader, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        print(f'LMDB Accuracy: {100 * correct / total}%')
        if prof:
            prof.stop()
            prof.export_chrome_trace("lmdb-trace.json")

@time('visualtransformer-lmdb')
def main():
    folder = 'data-formats/lmdb/'
    with LMDBDataset(folder + 'train_images', transform=transform) as train_dataset, \
         LMDBDataset(folder + 'val_images', transform=transform) as val_dataset:
    ...
    # torch.save(model.state_dict(), 'vit_b_16_imagenet_lmdb.pth')
